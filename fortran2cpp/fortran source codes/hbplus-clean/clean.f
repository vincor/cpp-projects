...
C     +          logpl, dirpl, nodepl, outlen
      integer   fistat, namlen, iocode, i, ii,
     +          outlen
CHECK v.2.1<--
c
...
C         (6,*)'Error in GETSTR: Ifail = ', ifail
C         stop
C      end if
CHECK v.1.0<--
...
c    +          hedend
c
      integer   atcnt, attype, i, j, k, n, aacnt, wkpl, clslen, cmplen, 
...
c               write (outfi, 6000) altlab, altsci(3,j), scatom, 
c     +                             altscc(j), wkres, seqbcd(nxres), 
c     +                            (altsca(k,j), k=1,3), altscr(1,j), 
c     +                             altscr(2,j)               
            write (outfi, 6000) altlab, altmci(3,i), mcatom, altmcc(i),
     +                          wkres, seqbcd(nxres),
     +                         (altmca(k,i), k=1,3), altmcr(1,i), 
     +                          altmcr(2,i)
CHECK v.2.1<--
