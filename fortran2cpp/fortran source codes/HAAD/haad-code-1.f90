...
program main
    ...
    integer,dimension(1000)::HH1,indx !!this record the atoms index for 1H within 5A, indx the sorted index
    real,dimension(1000)::rHH1 !!this for sorting index
    ...
    real angg,fbd,bd,r1
    !***************************************************************real r12,r23,r13
    real bx,by,bz,cx,cy,cz
    real ctbcx,ctbcy,ctbcz
    
    integer ipp(3),ipp0(3)
    integer ires,iatom,in,icc,ip2,ip3,ip4
    integer ip
    ...
    integer i,j,k,ii,kx,kx0
    integer nres,natom !number of atoms, angles, hbond bin width
    integer iseed
    integer i1,i2,i3,i4,i7
...
data atomcontype/"CT1","CT2","CT3","C","CC","CPH1","CPH2","CPT","N","NH1","NH2","NH3","NC2","CY","NR2","NY","H","HA","HB","HC","HS","HR1","CA","HR3","HP","O","OC","OH1","S","CP1","CP2","CP3","NP"/  ! atom types 8+8+9+4
   ...
   call getarg(1,f1)
    ...
    !!!!!!!!!!!!
    773 format(4F8.3,4I5,I3,3F8.3,I3)   !TEST!!!!
    !!!!!!!!!!!!!
    1101 format(A100)
    1102 format(A30)
    ...
    iseed=natom 
     ...
     !     if(xx(i)>990.0)cycle
!!!!
!          write(10,773) bond(i,1),bond(i,2),bond(i,3),bond(i,4),bonda(i,1),bonda(i,2),bonda(i,3),bonda(i,4),bonda(i,5),xx(i),yy(i),zz(i),ASSrd(i)
...
subroutine HEAVY_ADD
    ...
    if(abc.eq." CG ")then
       ...
       if(resd(i).eq."PHE".or.resd(i).eq."TYR")then !.or.resd(i).eq."TYR"))then
          ...
          yi=yy(i+5)-yy(i)
          zi=zz(i+5)-zz(i)
!!!!!!!!!!Added this line because record is changed but flag is missed         
          ASSrd(i+5)=1
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!          
          fbd=1.191/r1
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          if (i+2==5063) print*, i+2, "-> ", atomty(i+2),xx(i+2), yy(i+2), zz(i+2)       
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!            
...
!!!!!!!!!!Changed this line because flag needs to be changed         
          ASSrd(i+1)=1
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!           
!          ASSrd(i)=1
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!          
...
!!!!!!!!!!Changed this lines because record need to set flag correctly         
             ASSrd(i+1)=1
             ASSrd(i+2)=1
             ASSrd(i+3)=1
             ASSrd(i+4)=1
...
subroutine getresin
    ...
    !!   do kkk=2571, 2600
    !!     print*, "bond[", kkk, "]-> ", bond(kkk,1), bond(kkk,2), bond(kkk,3), bond(kkk,4)  
    !!   enddo   
